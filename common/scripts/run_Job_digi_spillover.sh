...
#RUN_COMMAND_GAUSS="$RICH_BASE_SOFTWARE/GaussDev_v53r2/build.${CMTCONFIG_GAUSS}/run -s GITCONDDBPATH=$RICH_BASE_SOFTWARE"
...
if [[ $RUN_BOOLE == "1" ]]; then
    ...
    for i in $(seq 0 $((${NUM_JOBS} - 1))); do
        ...
        echo "myInputFile = [ InputArea+\"/Gauss_${i}\", InputArea+\"/Gauss_$(($i + 250))\", InputArea+\"/Gauss_$(($i + 500))\", InputArea+\"/Gauss_$(($i + 750))\" ]" >>${OPTIONS_DIR_TMP}/${OPTIONS_FILE}_${i}.py
        echo "" >>${OPTIONS_DIR_TMP}/${OPTIONS_FILE}_${i}.py
