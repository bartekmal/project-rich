...
GlobalPID::PIDType GlobalPID::getMcType() const {
  if ( 0 == MCParticleType ) {
    return NoMCInfo;
  } else if ( 11 == MCParticleType || 11 == MCParticleType ) {
    return Electron;
  } else if ( 13 == MCParticleType || -13 == MCParticleType ) {
    return Muon;
  } else if ( 211 == MCParticleType || -211 == MCParticleType ) {
    return Pion;
  } else if ( 321 == MCParticleType || -321 == MCParticleType ) {
    return Kaon;
  } else if ( 2212 == MCParticleType || -2212 == MCParticleType ) {
    return Proton;
  } else {
...
bool GlobalPID::passBasicPIDDet() const {
  if ( config.mustHaveAnyRICH || config.mustHaveAerogel || config.mustHaveR1Gas || config.mustHaveR2Gas ) {
...
bool GlobalPID::passTrackSelection() const {
  ...
  // momentum cuts
  if ( TrackP > config.maxP ||   //
       TrackP < config.minP ||   //
       TrackPt > config.maxPt || //
       TrackPt < config.minPt ) {
    return false;
  }
  ...
  // no long valid for upgrade tracks
  // if ( TrackLikelihood > config.maxTrackLikelihood || //
  // TrackLikelihood < config.minTrackLikelihood ) {
  //   return false;
  // }
...
void GlobalPID::makeCurve( const Long64_t nTracks ) {
  ...
  for ( Long64_t jentry = 0; jentry < nentries; ++jentry ) {
    ...
    for ( unsigned int iStep = 0; iStep < config.nSteps; ++iStep, cut += dllStep ) {
      ...
      if ( selected ) {
        ...
        if ( Muon != config.idType || Muon != mcType || hasMuonInfo() ) ++data[mcType].second;
      }
    }
  ...
  if ( !config.superImpose ) { figures[figName] = canvas; }
  ...
  for ( auto iStep = pidData.begin(); iStep != pidData.end(); ++iStep, ++iLabel ) {
    // ID
  ...
  if ( !ideff.empty() ) {
    ...
    // make a graph
    TGraphErrors* g = new TGraphErrors( ideff.size(), &*ideff.begin(), &*misideff.begin(), &*idefferr.begin(),
                                        &*misidefferr.begin() );
    g->SetMarkerStyle( 21 );
    g->SetMarkerSize( 0.5 );
    ...
    std::cout << "WARNING : Zero data points found for " << name( config.idType ) << " v " << name( config.misidType )
              << " -> No graph created" << std::endl;
  }
}
...
std::string GlobalPID::name( const GlobalPID::Variable var ) const {
  ...
  case bremPIDe:
    return "BremPIDe";
  case caloPIDe:
    return "EcalPIDe+HcalPIDe+PrsPIDe+BremPIDe";
  case caloPIDmu:
    return "EcalPIDmu+HcalPIDmu";
  case muonNShared:
    return "MuonNShared";
  default:
    return "Unknown";
...
double GlobalPID::variable( const Variable var ) const {
  ...
  case bremPIDe:
    return BremPIDe;
  case caloPIDe:
    return caloTotalPIDe();
  case caloPIDmu:
    return caloTotalPIDmu();
  case muonNShared:
    return muonSharedHits();
  default:
    return 0;
  }
}
...
std::string GlobalPID::imageName() const {
  if ( config.imageFile.empty() ) {
    ...
    if ( config.secvar1 != NoVar ) {
      ...
      if ( config.secvar1 != NoVar ) { title << "-" << name( config.secvar2 ); }
...
void GlobalPID::configReset() { config = Configuration(); }

void GlobalPID::reset() {
  // delete the current canvases
  for ( CanvasData::iterator i = figures.begin(); i != figures.end(); ++i ) { delete i->second; }
  figures.clear();
  // reset the config
  configReset();
}
...
GlobalPID::GlobalPID( const std::string& filename ) { loadTTree( filename ); }

GlobalPID::~GlobalPID() { resetFile(); }
...
Long64_t GlobalPID::LoadTree( Long64_t entry ) {
  ...
  if ( !fChain ) return -5;
  auto centry = fChain->LoadTree( entry );
  if ( centry < 0 ) return centry;
  if ( !fChain->InheritsFrom( TChain::Class() ) ) return centry;
  auto* chain = (TChain*)fChain;
...
void GlobalPID::saveImage( TCanvas* canvas, const std::string& filename ) {
  if ( canvas ) {
