...
public: // definitions
  /// MC type
  enum PIDType { UnknownParticle = -2, NoMCInfo = -1, Electron = 0, Muon = 1, Pion = 2, Kaon = 3, Proton = 4 };

  /// Tracktype
  enum TrkType { TypeUnknown = 0, Velo, VeloR, Long, Upstream, Downstream, Ttrack };
...
private: // methods
  ...
  void saveImage( TCanvas* canvas, const std::string& filename );
...
private: // inline methods
  ...
  /// The Muon DLL
  inline double muonDLL() const { return ( 1 == MuonIsMuon ? MuonMuLL : -99999 ); }

  /// The Muon N Shared Hits
  inline double muonSharedHits() const { return ( 1 == MuonInAcc ? MuonNShared : 0 ); }

  /// The Combined Muon DLL
  inline double combinedMuonDLL() const { return ( 1 == MuonIsMuon ? CombDLLmu : -99999 ); }
  ...
  inline bool hasAerogelInfo() const { return RichUsedAero != 0; }

  /// RICH1 Gas info is available
  inline bool hasRich1Gas() const { return RichUsedR1Gas != 0; }

  /// RICH2 Gas info is available
  inline bool hasRich2Gas() const { return RichUsedR2Gas != 0; }
  ...
  /// CALO Ecal info ?
  inline bool hasEcalInfo() const { return InAccEcal == 1; }
  /// CALO Hcal Info ?
  inline bool hasHcalInfo() const { return InAccHcal == 1; }
  /// CALO Prs Info
  inline bool hasPrsInfo() const { return InAccPrs == 1; }
  /// CALO Brem Info ?
  inline bool hasBremInfo() const { return InAccBrem == 1; }
  ...
  // test the cut
  bool testCut( const double var, const double cut ) const { return ( config.cutType == ">" ? var > cut : var < cut ); }

public:
  /// Utility class to add labels to the eff v pur curves
  class Label {
  public:
    Label( double _x, double _y, const std::string& _label ) : x( _x ), y( _y ), label( _label ) {}
    double      x, y;
    std::string label;
  };
